...
# "entityType": {
#       "type": "text",
#       "label": "Entity type (e.g., AirQualityObserved)",
#       "placeholder": "entityType",
#       "mandatory": true
#     },
#     "entities": {
#       "type": "text",
#       "label": "Entity IDs available for this entity type (separated by comma)",
#       "placeholder": "entities",
#       "mandatory": false
#     },
#     "attributes": {
#       "type": "text",
#       "label": "Attributes available for these entities (separated by comma)",
...
class HistoricalPlugin(Plugin):
    ...
    def on_post_product_spec_validation(self, provider, asset):
        ...
        if 'servicePath' in asset.meta_info:
            servicePath = asset.meta_info['servicePath']
        if 'idPattern' in asset.meta_info:
            idPattern = asset.meta_info['idPattern']
        # if 'entityType' in asset.meta_info:
        #     entityType = asset.meta_info['entityType']
        # if 'entities' in asset.meta_info:
        #     entities = asset.meta_info['entities']
        # if 'service' in asset.meta_info:
        #     attributes = asset.meta_info['attributes']
        ...
        try:
            ...
            s = idPattern.split(":")
            if (len(s) < 6):
                raise PluginError("idPattern not valid")
            if (len(s) > 6):
                resource = s[6]
            resource_type = s[2]
            attr = ""
            
            #if (s.find('/v2/entities') == -1 and len(s.split('/')) < 6):
            #    raise PluginError("URL not valid")

            #if 'type' in parsed_query:
            #    resource_type = parsed_query['type'][0]
            #else:
            #    raise PluginError("URL not valid, entity type missing")
        ...
        # asset.meta_info['entityType'] = entityType
